	...
	public Object aroundLock(ProceedingJoinPoint point, Lock lock) throws Throwable {
		...
		try {
			...
			if (lock.waitTime() > 0) {
				...
				lockObj = distributedLock.tryLock(lockKey, lock.waitTime(), lock.leaseTime(), lock.unit(),
						lock.isFair());
			}
			else {
