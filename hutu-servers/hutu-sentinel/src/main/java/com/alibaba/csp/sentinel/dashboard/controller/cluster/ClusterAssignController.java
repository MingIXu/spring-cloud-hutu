...
public class ClusterAssignController {
	...
	@Autowired
	@Qualifier("clusterAssignNacosProvider")
	private DynamicRuleProvider<List<ClusterAppAssignMap>> clusterAssignNacosProvider;

	@Autowired
	@Qualifier("clusterAssignNacosPublisher")
	private DynamicRulePublisher<List<ClusterAppAssignMap>> clusterAssignNacosPublisher;
	...
	public Result<ClusterAppAssignResultVO> apiAssignAllClusterServersOfApp(@PathVariable String app,
		...
		try {
			// push 到 nacos
			clusterAssignNacosPublisher.publish(app, assignRequest.getClusterMap());
	...
	public Result<ClusterAppAssignResultVO> apiAssignSingleClusterServersOfApp(@PathVariable String app,
		...
		try {
			// push 到 nacos
			clusterAssignNacosPublisher.publish(app, Collections.singletonList(assignRequest.getClusterMap()));
	...
	public Result<ClusterAppAssignResultVO> apiUnbindClusterServersOfApp(@PathVariable String app,
		...
		try {
			// 删除解绑的配置再 push 到 nacos
			List<ClusterAppAssignMap> rules = clusterAssignNacosProvider.getRules(app);
			rules.removeIf(obj -> machineIds.contains(obj.getMachineId()));
			clusterAssignNacosPublisher.publish(app, rules);
