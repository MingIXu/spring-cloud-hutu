...
public class ESMetricsRepository implements MetricsRepository<MetricEntity> {
	...
	public void save(MetricEntity metric) {
		...
		// 结果集群数据重复问题
		String id = metric.getApp() + "_" + metric.getResource() + "_" + (metric.getTimestamp().getTime() / 1000);
		esMetric.setId(id);
		// 保存监控数据到ES中
	...
	public List<MetricEntity> queryByAppAndResourceBetween(String app, String resource, long startTime, long endTime) {
		...
		List<ESMetric> esMetrics = elasticsearchOperations.search(query, ESMetric.class).get()
	...
	public List<String> listResourcesOfApp(String app) {
		...
		// 多条件查询设置
		MatchPhraseQueryBuilder mpqb1 = QueryBuilders.matchPhraseQuery("app", app);
		// MatchPhraseQueryBuilder mpqb2 = QueryBuilders.matchPhraseQuery("resource",
		// resource);
		// 设置时间过滤
